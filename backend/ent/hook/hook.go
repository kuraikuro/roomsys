...
// The RoomStatusFunc type is an adapter to allow the use of ordinary
// function as RoomStatus mutator.
type RoomStatusFunc func(context.Context, *ent.RoomStatusMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RoomStatusFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.RoomStatusMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RoomStatusMutation", m)
	}
	return f(ctx, mv)
}
