...
// The RoomStatusQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type RoomStatusQueryRuleFunc func(context.Context, *ent.RoomStatusQuery) error

// EvalQuery return f(ctx, q).
func (f RoomStatusQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RoomStatusQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.RoomStatusQuery", q)
}

// The RoomStatusMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type RoomStatusMutationRuleFunc func(context.Context, *ent.RoomStatusMutation) error

// EvalMutation calls f(ctx, m).
func (f RoomStatusMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.RoomStatusMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.RoomStatusMutation", m)
}
