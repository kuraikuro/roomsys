...
type RoomQuery struct {
	...
	// eager-loading edges.
	withRoomstatus *RoomStatusQuery
...
// QueryRoomstatus chains the current query on the roomstatus edge.
func (rq *RoomQuery) QueryRoomstatus() *RoomStatusQuery {
	query := &RoomStatusQuery{config: rq.config}
...
// QueryRoominfo chains the current query on the roominfo edge.
func (rq *RoomQuery) QueryRoominfo() *RoomInfoQuery {
	query := &RoomInfoQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithRoomstatus tells the query-builder to eager-loads the nodes that are connected to
// the "roomstatus" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoomstatus(opts ...func(*RoomStatusQuery)) *RoomQuery {
	query := &RoomStatusQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomstatus = query
	return rq
}
...
//  WithRoominfo tells the query-builder to eager-loads the nodes that are connected to
// the "roominfo" edge. The optional arguments used to configure the query builder of the edge.
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [3]bool{
			rq.withRoomstatus != nil,
			rq.withRoomtype != nil,
			rq.withRoominfo != nil,
		}
	)
	if rq.withRoomstatus != nil || rq.withRoomtype != nil || rq.withRoominfo != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withRoomstatus; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_status_room; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(roomstatus.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_status_room" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Roomstatus = n
			}
		}
	}
	...
	if query := rq.withRoomtype; query != nil {
		...
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_type_room; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_type_room" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withRoominfo; query != nil {
		...
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_info_room; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_info_room" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Roominfo = n
			}
		}
	}
